...
ControlSignals D_ctrl, D_ctrl_dup;
reg [31:0] D_pc;
...
Decoder #(.IMPLEMENT_LIKELY(IMPLEMENT_LIKELY)) D_dec (
    .instruction(D_instr),
    .reset(reset),
...
ArithmeticLogicUnit D_alu(
                        .ctrl(D_ctrl.aluCtrl),
                        .A(D_ctrl.aluSrcA ? D_ctrl.immediate : D_regRead1_forward.value),
...
always_comb begin
    ...
    if (D_last_bubble) begin
        ...
        case (D_ctrl_dup.generateException)
...
wire [31:0] D_bitCounter_val = D_ctrl.bitCounterType ? D_regRead1_forward.value : ~D_regRead1_forward.value;
...
always @(posedge clk) begin
    if (reset) begin
        ...
        if (!E_stall) begin
            ...
            E_ctrl <= (D_insert_bubble || exceptionLevel[m_E] || D_exception) ? kControlNop : D_ctrl_dup;
            ...
            E_instr <= D_instr;
            E_bitCounter_val <= D_bitCounter_val;
        end
        else begin
...
ForwardController E_regRead1_forward (
                      ...
                      .original(E_regRead1),
                      .enabled(E_ctrl.mulCtrl != `mtDisabled),
...
ForwardController E_regRead2_forward (
                      ...
                      .original(E_regRead2),
                      .enabled(E_ctrl.mulCtrl != `mtDisabled || E_ctrl.writeCP0 || E_ctrl.memStore || E_ctrl.memUnaligned),
...
assign cp0_wdata = E_regRead2_forward.value;
assign tlb_op = E_ctrl.tlbop;

BitCounter E_bitCounter(
    .val(E_bitCounter_val) // forwarding is done in D to reduce delay
);
...
always @(posedge clk) begin
    if (reset) begin
        ...
        M_bitCount <= E_bitCounter.result;
