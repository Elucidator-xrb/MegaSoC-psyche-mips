...
logic [SET_ASSOC - 1 : 0]w_ram_valid;
...
always_comb begin
	...
	w_idle_miss = (r_state == IDLE || r_state == IDLE_RECEIVING) && w_tag_num && ~w_idle_hita && ~w_waita && (&w_ram_valid);
	w_state = r_state;
	case (r_state)
		IDLE:begin
...
assign o_ready = r_state != REFILL && r_state != INVALIDATING && w_memread_start != 1 && (&w_ram_valid);
...
for(genvar i = 0; i < SET_ASSOC; ++i) begin : iswayhit
	assign w_way_hita[i] = w_tag_res[i].tag == get_tag(w_phy_addr) && w_tag_res[i].valid && w_ram_valid[i]; 
